...
local AvailableFilters = { --Add more filters here
	["Lua Cache"] = { "dua", },
	["Map Files"] = { "bsp", "ain", "nav", },
	["Sound Files"] = { "wav", "ogg", "mp3", },
	["Model Files"] = { "mdl", "phy", "vtx", "vvd",	},
	["Texture Files"] = { "vtf", "vmt", },
	["Text Files"] = { "txt", "ini", "cfg", },
	["Particle Files"] = { "pcf", },
	["Miscellaneous"] = { "vcs", "ani", "jpg", },
}
...
if IsValid( extensionspanel ) then -- Failsafe incase garry ever removes the extensions menu
	...
	extensionspanel.PropertySheet:AddSheet( Localize( "Download Filter" ), DownloadFilter, "gui/silkicons/box" )
...
hook.Add( "ShouldDownload", "DownloadFilter", function( filename )
	...
	local ext = string.GetExtensionFromFilename( filename:gsub( ".bz2", "" ) ) -- Don't think this issue was ever addressed in the older GUI, the .bz2 was added after the filetype extentsion confusing our filter
	
	if ActiveFilters[ ext ] != nil and ActiveFilters[ ext ] == false then -- Shouldnt ever be nil
		console.AddText( Color( 255, 174, 201 ), "DownloadFilter", Color( 255, 255, 255 ), ": " .. filename ) -- Print out the blocked file to console all pretty like
