...
#include "common/GameUI/IGameUI.h" // IGameUI interface
#include "common/GameUI/IGameConsole.h" // IGameConsole interface
...
LUA_FUNCTION( WriteSaveGameScreenshotOfSize )
{
	...
	bool unknwn1 = Lua()->GetBool(4);
	bool unknwn2 = Lua()->GetBool(4);

	g_BaseClientDLL->WriteSaveGameScreenshotOfSize( Lua()->GetString(1), Lua()->GetNumber(2), Lua()->GetNumber(3), ( unknwn1 != NULL ? unknwn1 : false ), ( unknwn2 != NULL ? unknwn2 : false ) );
	return 0;
}
...
int Open( lua_State *L ) {
	...
	if ( !Lua()->IsClient() )
		Lua()->Error( "gmcl_extras can only be loaded on the client!" );
